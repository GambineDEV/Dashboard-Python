...
# --- Função: ISO3 → Nome do País ---
def iso3_para_pais(iso3):
    try:
        return pycountry.countries.get(alpha_3=iso3).name
    except:
        return iso3  # fallback se não encontrar

# --- Carregamento dos dados ---
df = pd.read_csv("dados_com_paises_pt.csv")

# Cria coluna com nome do país
df["pais_nome"] = df["residencia_iso3"].apply(iso3_para_pais)
...
with col_graf4:
    if not df_filtrado.empty:
        ...
        media_ds_pais = (
            df_ds
            .groupby(['residencia_iso3', 'pais_portugues'])['usd']
            .mean()
            .reset_index()
        )
        ...
        grafico_paises = px.choropleth(
            ...
            color='usd',
            hover_name='pais_portugues',
            hover_data={
                'pais_portugues': False,
